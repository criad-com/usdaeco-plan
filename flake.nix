{
  description = "usdAecoPlan semantic library and example";
  inputs = {
  ...
  outputs = { self, nixpkgs, toolchain, core, axis, datacentre }:
    let
      ...
      forSystem = system:
        let
          ...
          schema = kit.buildCodelessSchema { name = "usdAecoPlan"; src = self; deps = [ corePlugin ]; };
          ...
          render = pkgs.writeShellApplication {
            ...
            runtimeInputs = [ kit.pythonEnv kit.usd-dev ];
            text = setup + ''
              env -u PYTHONPATH usdaeco-render ${self}/usdAecoPlan/examples/minimal.usda \
    ...
    in {
      ...
      checks = eachSystem (system: let p = forSystem system; in {
        library = p.pkgs.runCommand "usdAecoPlan-check" {
          ...
          mkdir -p "$out"
        '');
        structure = p.pkgs.runCommand "usdAecoPlan-structure" {
